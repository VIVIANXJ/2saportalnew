...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (q?.trim()) {
      query = query.or(`name.ilike.%${q.trim()}%,company.ilike.%${q.trim()}%`);
    }
  ...
  if (req.method === 'POST') {
    const { name, company = '', address1 = '', address2 = '', suburb = '',
            state = '', postcode = '', country = 'AU', phone = '', email = '', notes = '' } = req.body || {};
    if (!name?.trim()) return res.status(400).json({ error: 'Name is required' });
    const { data, error } = await supabase
      .from('locations')
      .insert({ name: name.trim(), company, address1, address2, suburb, state, postcode, country, phone, email, notes })
      .select().single();
    if (error) return res.status(500).json({ error: error.message });
  ...
  if (req.method === 'PATCH') {
    ...
    const fields = ['name','company','address1','address2','suburb','state','postcode','country','phone','email','notes','active'];
    const updates = {};
