...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    const { name, company = '', address1 = '', address2 = '', suburb = '',
            state = '', postcode = '', country = 'AU', phone = '', email = '', instructions = '', special_instruction = '',
            receiver_code = '', mobile = '', billing_group = '' } = req.body || {};
    if (!name?.trim()) return res.status(400).json({ error: 'Name is required' });
    const { data, error } = await supabase
      .from('locations')
      .insert({ name: name.trim(), company, address1, address2, suburb, state, postcode, country, phone, email, instructions, special_instruction, receiver_code, mobile, billing_group })
      .select().single();
    if (error) return res.status(500).json({ error: error.message });
  ...
  if (req.method === 'PATCH') {
    ...
    const fields = ['name','company','address1','address2','suburb','state','postcode','country','phone','email','instructions','special_instruction','receiver_code','mobile','billing_group','active'];
    const updates = {};
